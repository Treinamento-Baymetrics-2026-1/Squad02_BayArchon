...
const userModalSchema = z.object({
  ...
  sector_id: z
    ...
    .refine((val) => val !== "0", "Escolha um setor."),
});
    ...
    >
      <form className="space-y-4" onSubmit={handleSubmit(onSubmit)}>
        <div className="space-y-1.5">
          ...
          <div className="relative">
            <input
              ...
              className={`w-full h-11 px-4 border rounded-xl text-preto-suave focus:outline-none focus:ring-1 text-[15px] placeholder:text-cinza-escuro ${
                errors.name
                  ? "border-vermelho focus:border-vermelho focus:ring-vermelho"
                  : "border-bordaoff-white focus:border-azul-interativo focus:ring-azul-interativo"
              }`}
              {...register("name")}
            />
            {isEditing && (
          ...
          {errors.name && (
            <p className="text-sm text-vermelho">{errors.name.message}</p>
          )}
        </div>
        ...
        <div className="space-y-1.5">
          ...
          <input
            ...
            className={`w-full h-11 px-4 border rounded-xl text-preto-suave focus:outline-none focus:ring-1 text-[15px] placeholder:text-cinza-escuro transition-all${
              isEditing
                ? "bg-off-white text-cinza-escuro/70 cursor-not-allowed border-bordaoff-white"
                : errors.cpf
                  ? "border-vermelho focus:border-vermelho focus:ring-vermelho"
                  : "border-bordaoff-white focus:border-azul-interativo focus:ring-azul-interativo"
            }`}
          ...
          />
          {errors.cpf && (
            <p className="text-sm text-vermelho">{errors.cpf.message}</p>
          )}
        </div>
        ...
        <div className="space-y-1.5">
          ...
          {errors.email && (
            <p className="text-sm text-vermelho">{errors.email.message}</p>
          )}
        </div>
        ...
        <div className="space-y-1.5">
          ...
          {errors.sector_id && (
            <p className="text-sm text-vermelho">{errors.sector_id.message}</p>
          )}
        </div>
